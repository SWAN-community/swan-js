...
describe('salt', () => {
  test('pass', async () => {
    await testPass<Salt>(createSalt);
  });
  test('fail wrong signer', async () => {
    await testWrongSigner<Salt>(createSalt);
  });
  test('serialize base 64', async () => {
    await testBase64<Salt>(createSalt);
  });
  test('serialize JSON', async () => {
    await testJson<Salt>(createSalt, (s) => new Salt(s));
  });
  test('fail wrong value', async () => {
    const a = await createArtifact();
    const e = await createSigned<Salt>(a.signer, createSalt);
    const b = e.valueByteArray;
    b[0]++;
    e.valueByteArray = b;
